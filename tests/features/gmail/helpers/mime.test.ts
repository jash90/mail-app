import {
  ...
  extractBody,
  createRawEmail,
} from '@/features/gmail/helpers/mime';
import type { GmailMessagePayload } from '@/features/gmail/types';
...
describe('extractBody', () => {
  ...
  it('extracts both text and html from multipart payload', () => {
    const textEncoded = Buffer.from('Plain text').toString('base64')
      .replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    const htmlEncoded = Buffer.from('<b>HTML</b>').toString('base64')
      .replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
