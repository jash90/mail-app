...
/** Registered by the AI subsystem at startup to check if local AI is the active provider. */
let isLocalAI: (() => boolean) | null = null;

/** Register the local AI check callback. Called once during app initialization. */
export function registerLocalAICheck(fn: () => boolean): void {
  isLocalAI = fn;
}
...
export async function acquireNetwork(
  ...
  // Only wait if local AI is active (check via registered callback)
  if (aiActive && isLocalAI?.()) {
