...
import {
  getSummaryCache,
  summarizeEmail,
} from '@/src/features/ai/services/api';
import {
...
/** Injected Gmail dependencies — avoids cross-feature imports. */
export interface SummaryPipelineDeps {
  getInboxUnreadCount: () => Promise<number>;
  syncLabelThreads: (accountId: string, labelIds: string[]) => Promise<unknown>;
}

export function useSummaryPipeline(
  accountId: string,
  userEmail: string,
    ...
    const handle = InteractionManager.runAfterInteractions(() => {
      (async () => {
        try {
          ...
          try {
            serverCount = await deps.getInboxUnreadCount();
          } catch {
          ...
          if (serverCount !== null && serverCount !== localCount) {
            ...
            try {
              await deps.syncLabelThreads(accountId, ['INBOX']);
            } catch {
  ...
  }, [accountId, userEmail, runId, updatePhase, loadReplacementThread, deps]);
