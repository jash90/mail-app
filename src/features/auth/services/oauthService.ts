import { GOOGLE_AUTH } from '@/src/shared/config/constants';
import { Sentry } from '@/src/shared/services/sentry';
import { AuthError } from '@/src/shared/services/errors';
import { GoogleUser } from '@/src/shared/store/authStore';
...
/** Perform interactive Google Sign-In. Returns user payload + access token.
 *  Wraps GoogleSignin so screens never import the SDK directly.
 */
export async function signInWithGoogle(): Promise<{
  ...
  if (!data?.user)
    throw new AuthError('SIGN_IN_CANCELLED', 'User cancelled sign-in');
