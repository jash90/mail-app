...
/** Callback to reset FTS search verification flag after rebuild. Injected by the app layer. */
let onFtsRebuilt: (() => void) | null = null;
...
/** Configure a callback invoked after FTS index is rebuilt. */
export function setOnFtsRebuilt(callback: (() => void) | null): void {
  onFtsRebuilt = callback;
}
