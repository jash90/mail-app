...
import type { ThreadLengthBucket } from '@/features/stats/types';
...
export default function ThreadLengthChart({ buckets, average, median }: Props) {
  ...
  return (
    <View className="mt-4">
      <Text className="mb-1 text-lg font-semibold text-white">Thread Lengths</Text>
      ...
      {buckets.map((bucket) => (
        <View key={bucket.label} className="mb-1.5 flex-row items-center">
          ...
          <View className="flex-1 flex-row items-center">
            <View className="h-4 overflow-hidden rounded bg-zinc-800" style={{ flex: 1 }}>
              <View
